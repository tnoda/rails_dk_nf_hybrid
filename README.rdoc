...
* Use the *person_name* column table as an additional foreign key in the *comment* table,
  and the *name* column as a secondary key in the *people* table.
    ...
    class Comment
      belongs_to :person
      additional_foreign_key :person, :secondary_key => :name, :foreign_key => :person_name,
    end
...
<tt>additional_foreign_key</tt> class-method simply defines a <tt>before_save</tt> callback
